...
//эти значения вычитаются из сгенерированных значений, чтобы придать форму анимации
...
//это оттенки для огня,
//должно быть от 0 (красный) до примерно 25 (желтый)
...
// Случайным образом генерируем следующую строку (строку матрицы)
void generateLine() 
{
...
// нарисуем кадр, интерполируя между 2-я "ключевыми кадрами"
// pcnt - процент интерполяции

void drawFrame(int pcnt) 
{
  int nextv;
  //каждая строка интерполируется с предыдущей  
...
//первая строка интерполируется с "следующей" строкой  
  for (unsigned char x = 0; x < WIDTH; x++) 
  {
    uint8_t newX = x;
    if (x > 15) newX = x - 15;
