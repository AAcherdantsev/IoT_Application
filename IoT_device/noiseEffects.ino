...
// 16-битная версия координат

static uint16_t x;
static uint16_t y;
static uint16_t z;

uint16_t speed = 20; // скорость устанавливается динамически после запуска
uint16_t scale = 30; // масштаб  устанавливается динамически после запуска

// Это массив, в котором мы храним наши вычисленные значения шума
...
void fillNoiseLED() 
{
  ...
  // применяем медленный дрейф к X и Y для визуального изменения  
  x += speed / 8;
  y -= speed / 16;
  ...
  for (int i = 0; i < WIDTH; i++) 
  {
    for (int j = 0; j < HEIGHT; j++) 
    {
      ...
      // если эта палитра является "циклом", добавим медленно изменяющееся базовое значение      
      if ( colorLoop) 
      {
        index += ihue;
      }
